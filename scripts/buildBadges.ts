...
fs.readdir(imagesDir, async (err, files) => {
  ...
  spritesmith.run({ src: resizedImagePaths }, (err, result) => {
    if (err) {
      console.error('Error generating badges spritesheet:', err);
      return;
    }

    // Write spritesheet image as JPG
    const spritesheetFilePath = path.join(outputDir, 'badges' + outputExt);
    // turn into jpg
    sharp(result.image)
      // .jpeg({ quality: 60 })
      .toBuffer()
      .then((buffer) => {
        ...
        console.log('Spritesheet generated badges successfully!');
      })
      .catch((err) => {
        console.error('Error saving badges spritesheet as JPG:', err);
      });
    ...
    console.log('Badges spritesheet generated successfully!');
  });
});
